...
# interaction plot - there does appear to be an interaction bwtee
...
# Regression model with indicators and without intercept. do we need this?
...
```
*ANOVA Discussion*
**We model grade with predictors Walc2 (binary weekend alcohol consumption) and sex with a two-way ANOVA model. Because the ratio of max/min sample SD for our predictors is 1.1, we can pass the assumption of normal distributed and constant variance across groups. In the model summary, we can see that all predictors (Walc2, sex, and the interaction between the two) are significant. We can conclude that there are differences in grade due to none or some alcohol consumption and male or female sex and there is a statistically signifcant interaction between these two factors.**

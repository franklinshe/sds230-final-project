...
Our data did not require much cleaning. We renamed the column G3 to "grade" to provide a more intuitive and clear description of what the variable was and we removed observations where grade was 0 because this unreasonable grade indicated that the data for these observations was incomplete. 
...
# Graphics - Scatterplot (grade and weekend and weekday alcohol consumption)
...
# Graphics - Histogram (grade, freetime, and going out)
...
**(See Matrix Plot for more correlations.)  **
...
```{r}
# Basic Tests - Permutation Test (grade by school)
...
**We perform backwards step-wise multiple regression predicting grade. Our plan is to include any interactions indicated by interaction plots and all variables (school, sex, age, studytime, higher education, freetime, goout, Dalc, Walc, health, absences, grade), then remove non-significant predictors in order of least significance to most significance, leaving only significant predictors. We transform Walc, Dalc, and absences as indicated and discussed in our matrix plot above. We begin with step-wise removal of interaction terms, then of the remaining predictors. Because we have categorical predictors, we use backwards stepwise regression instead of best subsets regression.**
...
##backwards step-wise regression, resulting model
mod2 <- lm(grade ~ Walc*sex + school + sex + age + studytime + higher + freetime + health + sqrtabs)
summary(mod2)

##boxcox (delete)
trans1 <- boxCox(mod2)
(trans1$x[which.max(trans1$y)]) ## close to 1! good ?
```
*Multiple Regression - Residual Plots*
Normal Quantile Plot of residuals, plot of fits vs. residuals, discussion
#### check on what the assumptions of multiple regression are
- residuals normally distr
- ind vars not correlated w eachother
...
# Comparing Pairs of Means in Two-Way ANOVA (turn into one-way anova and use Tukey)

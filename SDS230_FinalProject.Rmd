...
##does anyone know why it says NaN in the last column for tehe correlations with lobabs
...
```{r}
t.test(Walc ~ sex) ##
t.test(log10(Walc) ~ sex) ## stil significant
t.test(studytime ~ sex)
t.test(grade ~ sex)
t.test(grade ~ school)
t.test(Walc ~ school) ## not significant
t.test(log10(Walc) ~ school) ## not significant
